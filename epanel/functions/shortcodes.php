<?php
	...
	add_shortcode('member', 'rs_member_shortcode');
	function rs_member_shortcode( $atts ){
		ob_start();
		...
		// check the query
		if($query->have_posts()) { ?>
		<h1 class="text-center title"><?php echo $member_category; ?></h1>
					...
					<div class="mb20 featured-image">
						<figure>
							<?php
								...
								echo '<a href="'. get_permalink() .'" data-rel="fancybox">' . get_the_post_thumbnail( get_the_ID(), apply_filters( 'single_post_large_thumbnail_size', 'thumbnail' ), $attr ) . '</a>';
							?>
						</figure>
					</div>
...
<?php		}?>
		...
		<div class="row text-right">
			<div class="col-md-12">
				<span class="totalmember small">
					<?php echo $query->found_posts;?>(members)
				</span>
			</div>
		</div>
<?php	} ?>
